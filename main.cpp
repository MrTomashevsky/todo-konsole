/*
Напиши мне программу на С++ с использованием QString (для того, чтобы сохранять символы на кириллице в unicode), которая занимается обработкой записей в файл fileName.

Содержимое fileName - это массив записей вида:
...
{
 separatorRecord = "\033\033\033\033",
...
./program <имя json файла> <имя функции> <ее аргументы>


Предоставь код с подробными комментариями.
 */
...
1. **Структура DataRecord**:
- Хранит все поля одной записи, включая ID, текст, временные метки, статус и приоритет.

2. **Класс RecordManager**:
- Обрабатывает все операции с записями: загрузку, сохранение, создание, удаление, зачеркивание.
- Использует QString для корректной работы с кириллицей и Unicode.
- Поддерживает цветной вывод через escape-последовательности.

3. **Работа с файлами**:
- Записи сохраняются в текстовом файле с разделителями.
- Формат хранения: каждая запись - несколько строк, разделенных separatorLine, а записи разделены separatorRecord.

4. **Обработка команд**:
- `cross <id>` - переключает статус зачеркивания.
- `delete <id>` - удаляет запись.
- `create <degree> <text>` - создает новую запись.
- `list`, `list_nocross`, `list_cross` - вывод записей с разными фильтрами.

5. **Цветной вывод**:
- Используются ANSI escape-коды для окрашивания вывода.
- Цвета настраиваются через JSON-конфиг.

6. **JSON-конфигурация**:
- Позволяет настроить разделители, имя файла и цвета.
- Если конфиг не указан, используются значения по умолчанию.

Программа корректно обрабатывает кириллические символы, поддерживает цветной вывод в терминал и предоставляет все запрошенные функции для работы с записями.
*/
...
delete <id> - удалить запись;
create <degreeOfPriority> <text> - создать новую запись;
priority <id> <degreeOfPriority> - задать новую степень приоритетности.
Следующие 3 функции имеют похожие аргументы - произвольное количество пар <ключ@имя поля>. Ключ - строка либо "ask"/"desk", означающие параметры сортировка по полю "имя поля":
list <аргументы> - вывод всех записей;
list_nocross <аргументы>  - вывод незачеркнутых записей;
list_cross - вывод зачеркнутых записей.
...
./program <необязательно: имя json файла> <имя функции> <ее аргументы>
Предоставь код с подробными комментариями.
